...
class DataLoaderFE(object):
    ...
    def get_iterator(self, epoch=0):
        ...
        if self.unsupervised:
            ...
            # plus the label of the rotation, i.e., 0 for 0 degrees rotation,
            # 1 for 90 degrees, 2 for 180 degrees, and 3 for 270 degrees.
            # plus the four rotations with some fisheyeeffect for labels 4 - 7
            def _load_function(idx):
                ...
                if self.first_time:
                    ...
                    self.grid = torch.zeros([1, img_size, img_size, 2])
                ...
                # print(rotated_imgs[0].size())
                rotated_imgs = [
                    rotated_imgs[0],
                    rotated_imgs[1],
                    rotated_imgs[2],
                    rotated_imgs[3],
                    nn.functional.grid_sample(rotated_imgs[0].unsqueeze(0), self.grid, mode=random.choice(['nearest', 'bilinear', 'triliinear'])).data.squeeze(),
                    nn.functional.grid_sample(rotated_imgs[1].unsqueeze(0), self.grid, mode=random.choice(['nearest', 'bilinear', 'triliinear'])).data.squeeze(),
                    nn.functional.grid_sample(rotated_imgs[2].unsqueeze(0), self.grid, mode=random.choice(['nearest', 'bilinear', 'triliinear'])).data.squeeze(),
                    nn.functional.grid_sample(rotated_imgs[3].unsqueeze(0), self.grid, mode=random.choice(['nearest', 'bilinear', 'triliinear'])).data.squeeze()
                ]
                # rotation_labels = torch.LongTensor([0, 1, 2, 3])
